...
<head>
    ...
    <link href="{{asset('css/convert-pdf/css-spending.css')}}" rel="stylesheet">
    <script src="https://cdnjs.cloudflare.com/ajax/libs/d3/4.3.0/d3.min.js"></script>
    <title>Bukuku Personal</title>
</head>
<body class="report-spending">
{{--
                                ...
                                @foreach($content as $item)
                                    <ion-row class="list-item">
                                        <ion-col class="list-item-icon">
                                            <img
                                                src="{{ $item->spending->icon_id ? $item->spending->icon_url : asset('img/ic_wallet.svg')}}"
                                                class="col-icon">
                                        </ion-col>
                                        <ion-col class="list-item-note">
                                            <div> {{$item->spending->name}}</div>
                                            <div> {{$item->note}}</div>
                                        </ion-col>
...
<script>
    ...
    chartData = chartData.map((item) => {
        return {
            ...
            'icon_url': item.spending.icon_url ? item.spending.icon_url : ''
        }
    });
    ...
    createPieChart(chartData, 'spending');
